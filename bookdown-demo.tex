...
\urlstyle{same}
\hypersetup{
  pdftitle={Notas de matematicas},
  pdfauthor={Hector Henry Jurado Soto},
  hidelinks,
  pdfcreator={LaTeX via pandoc}}

\title{Notas de matematicas}
...
\protect\hypertarget{def:label}{}\label{def:label}El \emph{espacio muestral} \(S\) de un experimento es el conjunto de todos los posibles resultados de un experimento. Un \emph{evento} \(A\) es un subconjunto del espacio muestral \(S\) y se dice que \(A\) \emph{ocurrió} si el resultado actual pertenece a \(A\)
\end{definition}
...
\begin{example}[Lanzamiento de monedas]
\protect\hypertarget{exm:coins}{}\label{exm:coins}Una moneda es lanzada 10 veces
\end{example}
